...
"""def loading_lines_and_links_relative_quantified(
    ...
    return result_lines, result_links
"""
...
def loading_lines_and_links_relative_quantified(
    ...
    result_lines = pd.concat([
        ...
        type="AC Line",
        zone_bus0=lambda df: df["bus0"].map(bus_zones),
        zone_bus1=lambda df: df["bus1"].map(bus_zones)
    )
    ...
    # === Reorder columns (zones placed after buses) ===
    col_order = [
        "bus0", "zone_bus0",
        "bus1", "zone_bus1",
        "loading_percent", "loading_MW", "rated_capacity_MW", "type"
    ]
    result_lines = result_lines[col_order]
    result_links = result_links[col_order]
