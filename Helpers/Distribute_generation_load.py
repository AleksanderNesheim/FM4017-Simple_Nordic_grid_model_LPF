...
#--------------------
    # --- Total production per zone (row-wise) ---
    # Combine generators + storage (only positive discharge)
    gen_prod = n.generators_t.p_set.copy()
    stor_prod = n.storage_units_t.p_set.clip(lower=0).copy()

    all_prod = pd.concat([gen_prod, stor_prod], axis=1)

    # Map each column to its zone
    zone_map = pd.concat([
        n.generators["zone"],
        n.storage_units["zone"]
    ])

    # Sum per zone, per snapshot
    production_per_zone = all_prod.groupby(zone_map, axis=1).sum()

    print("\nTotal production per zone (MW):\n", production_per_zone)
    
        # --- Total load per zone (row-wise) ---
    load_df = n.loads_t.p_set.copy()

    # Map loads to their zones
    load_zone_map = n.loads["zone"]

    # Sum per zone per snapshot
    load_per_zone = load_df.groupby(load_zone_map, axis=1).sum()

    print("\nTotal load per zone (MW):\n", load_per_zone)

    # --- Zone-wise imbalance: production minus load ---
    # Align indexes and columns just to be safe
    imbalance_per_zone = production_per_zone.reindex(columns=load_per_zone.columns).subtract(load_per_zone, fill_value=0)

    print("\nZone-wise production - load (MW):\n", imbalance_per_zone)
